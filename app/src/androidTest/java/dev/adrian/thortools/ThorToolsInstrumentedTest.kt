...
class ThorToolsInstrumentedTest {
    ...
    fun debugBackendPresentsThorCapabilities() {
        ...
        assertEquals("0", snapshot.profile.properties.flashLocked)
        assertEquals("unlocked", snapshot.profile.properties.bootloaderDeviceState)
        assertEquals("orange", snapshot.profile.properties.verifiedBootState)
        assertTrue(snapshot.profile.properties.bootloaderUnlocked == true)
        assertTrue(snapshot.profile.supports(ThorCapability.BOOTLOADER_UNLOCKED))
