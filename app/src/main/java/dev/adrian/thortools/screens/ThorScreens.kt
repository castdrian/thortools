...
private fun DashboardIdentity(snapshot: ThorSnapshot) {
    Card(modifier = Modifier.fillMaxWidth()) {
        Column(modifier = Modifier.padding(16.dp), verticalArrangement = Arrangement.spacedBy(4.dp)) {
            ...
            DataLine("Bootloader", snapshot.profile.properties.bootloaderStateLabel)
            DataLine("flash.locked", snapshot.profile.properties.flashLocked)
            DataLine("vbmeta device state", snapshot.profile.properties.bootloaderDeviceState)
            DataLine("Verified boot state", snapshot.profile.properties.verifiedBootState)
