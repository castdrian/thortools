...
2. Choose **Download or open Magisk**. The first tap downloads the current Magisk APK; tap it again after the download completes to open Android's installer.
...
4. Choose **Prepare root patch**. ThorTools uses the active slot's `init_boot` image when that partition exists and uses `boot` only when `init_boot` is unavailable; it blocks if the selected target lacks a verified stock image or any currently available slot is missing a verified stock recovery source.
